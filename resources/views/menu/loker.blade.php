          ...
          <div class="inner-column">
            <div class="filters-outer">
              ...
              <div class="checkbox-outer">
                ...
                <ul class="checkboxes">
                    @foreach($jenis_pekerjaan as $jp)
                    <li>
                        <input type="checkbox" id="jp-{{$jp->id}}"
                        @if(@in_array($jp->id, @$parameter['jenis_pekerjaan']))
                        checked
                        @endif
                        name="jenis_pekerjaan[]" value="{{$jp->id}}">
          ...
          <div class="ls-outer">
             ...
             <div class="ls-switcher">
                <div class="showing-result">
                  ...
                  <div class="text"><strong>{{$data->firstItem()}}-{{$data->lastItem()}}</strong> dari <strong>{{$data->total()}}</strong> Lowongan Pekerjaan</div>
                </div>
            </div>
            @foreach($data as $d)
            <!-- Job Block -->
            <div onclick="location.href='{{url('menu/loker-detil?id='.$d->id)}}';" style="cursor: pointer;" class="job-block">
              <div class="inner-box">
                <div class="content">
                  <span class="company-logo"><img src="{{Sideveloper::storageUrl($d->foto_perusahaan)}}" alt=""></span>
                  <h4><a href="{{url('menu/loker-detil?id='.$d->id)}}">{{$d->judul}}</a></h4>
                  <ul class="job-info">
                    <li><span class="icon flaticon-map-locator"></span> {{$d->kecamatan}}, {{$d->kelurahan}}</li>
                    <li><span class="icon flaticon-clock-3"></span> Batas Akhir Lamaran: {{Sideveloper::date($d->tanggal_kadaluarsa)}}</li>
