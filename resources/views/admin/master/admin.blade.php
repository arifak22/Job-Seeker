 ...
 <section class="user-dashboard">
            ...
            <div class="tabs-box">
              ...
              <div class="widget-content">
                ...
                <form class="default-form" id="myform">
                    ...
                    {!!Sideveloper::formHidden('id_privilege', 3)!!}
                    {!!Sideveloper::formInput('Nama','text','nama')!!}
                    {!!Sideveloper::formInput('Password','password','password')!!}
                    {!!Sideveloper::formInput('Retype Password','password','password_2')!!}
                    {!!Sideveloper::formInput('E-mail','email','email')!!}
                    {!!Sideveloper::formFile('Foto', 'foto',  "accept=\"image/*\"", "Maksimal 200kb, Upload Gambar. (Image: png / jpg)")!!}
  ...
  <script>
    ...
    var tableList = $('#table_id').DataTable({
        ...
        // order        : [[2,'desc']],
        ajax         : {
            url  : "{{url('api/admin/master-admin')}}",
            type : "get",   
            headers: {
    ...
    function hapus(id){
        ...
        .then((willDelete) => {
            if (willDelete) {
                ...
                $.ajax({
                    method: "POST",
                    url:  "{{url('api/admin/master-admin-hapus')}}",
                    headers: {
                        "Authorization": "Bearer " + localStorage.getItem('jwt_token')
                    },
    ...
    function ubah(id){
        ...
        $.ajax({
            method: "GET",
            url:  "{{url('api/admin/master-admin-detil')}}",
            headers: {
                "Authorization": "Bearer " + localStorage.getItem('jwt_token')
            },
            data: { id: id, _token: "{{ csrf_token() }}" }
        ...
        .done(function(res) {
            ...
            $("#foto-name").html(`<a target="_blank" href="{{Sideveloper::storageUrl()}}/${res.data.foto}">
              <img style="height: 120px;margin-right:10px;" src="{{Sideveloper::storageUrl()}}/${res.data.foto}">
              </a>`);
        })
        .fail(function(err) {
            alert("error");
    ...
    $('#submit').click(function(e) {
        ...
        apiLoading(true, btn);
        form.ajaxSubmit({
            url    : "{{url('api/admin/master-admin-post')}}",
            type   : 'POST',
            headers: {
                "Authorization": "Bearer " + localStorage.getItem('jwt_token')
            },
