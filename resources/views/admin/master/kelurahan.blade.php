 ...
 <section class="user-dashboard">
    <div class="dashboard-outer">
      <div class="upper-title-box">
        <h3>{{$title}} ( Kecamatan {{$kecamatan->name}} )</h3>
      </div>
      <div class="row">
        <div class="col-lg-12">
            ...
            <div class="tabs-box">
              ...
              <div class="widget-content">
                ...
                <form class="default-form" id="myform">
                    ...
                    {!!Sideveloper::formHidden('district_id', $kecamatan->id)!!}
                    {!!Sideveloper::formInput('Nama Kelurahan','text','name')!!}
  ...
  <script>
    ...
    var tableList = $('#table_id').DataTable({
        ...
        // order        : [[2,'desc']],
        ajax         : {
            url  : "{{url('api/admin/master-kelurahan')}}",
            type : "get",   
            headers: {
                "Authorization": "Bearer " + localStorage.getItem('jwt_token')
            },
            data : {district_id: "{{$kecamatan->id}}"},
            error: function(){ 
                ...
                render: function ( data, type, row, meta ) {
                    return `<div class="option-box">
                              <ul class="option-list">
                                <li><button data-text="Hapus" onclick="hapus(${data}, ${row.district_id})"><span class="la la-trash"></span></button></li>
                                <li><button data-text="Ubah" onclick="ubah(${data}, ${row.district_id})"><span class="la la-pencil"></span></button></li>
                              </ul>
                            </div>`;
                }
            },
    ...
    function hapus(id, district_id){
        ...
        .then((willDelete) => {
            if (willDelete) {
                ...
                $.ajax({
                    method: "POST",
                    url:  "{{url('api/admin/master-kelurahan-hapus')}}",
                    headers: {
                        "Authorization": "Bearer " + localStorage.getItem('jwt_token')
                    },
                    data: { id: id, district_id: district_id, _token: "{{ csrf_token() }}" }
                })
                .done(function(res) {
                    apiRespone(res,
						null,
    ...
    function ubah(id, district_id){
        apiLoading(true);
        $.ajax({
            method: "GET",
            url:  "{{url('api/admin/master-kelurahan-detil')}}",
            headers: {
                "Authorization": "Bearer " + localStorage.getItem('jwt_token')
            },
            data: { id: id, district_id: district_id, _token: "{{ csrf_token() }}" }
        ...
        .done(function(res) {
            ...
            $("#alert").html(`<div style="position: relative;background: #F0F5F7;border-radius: 8px;padding:5px;">
                   <b class="blinking" style="color:#d62727"> Merubah Data ini akan merubah Kelurahan yang sudah di pilih Perusahaan, Pelamar Kerja & Loker </b>
                </div>
                <br>`);
    ...
    $('#submit').click(function(e) {
        ...
        apiLoading(true, btn);
        form.ajaxSubmit({
            url    : "{{url('api/admin/master-kelurahan-post')}}",
            type   : 'POST',
            headers: {
                "Authorization": "Bearer " + localStorage.getItem('jwt_token')
            },
