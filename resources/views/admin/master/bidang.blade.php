 ...
 <section class="user-dashboard">
            ...
            <div class="tabs-box">
              ...
              <div class="widget-content">
                ...
                <form class="default-form" id="myform">
                    ...
                    {!!Sideveloper::formInput('Nama Bidang','text','nama_bidang')!!}
  ...
  <script>
    ...
    var tableList = $('#table_id').DataTable({
        ...
        // order        : [[2,'desc']],
        ajax         : {
            url  : "{{url('api/admin/master-bidang')}}",
            type : "get",   
            headers: {
    ...
    function hapus(id){
        ...
        .then((willDelete) => {
            if (willDelete) {
                ...
                $.ajax({
                    method: "POST",
                    url:  "{{url('api/admin/master-bidang-hapus')}}",
                    headers: {
                        "Authorization": "Bearer " + localStorage.getItem('jwt_token')
                    },
    ...
    function ubah(id){
        ...
        $.ajax({
            method: "GET",
            url:  "{{url('api/admin/master-bidang-detil')}}",
            headers: {
                "Authorization": "Bearer " + localStorage.getItem('jwt_token')
            },
            data: { id: id, _token: "{{ csrf_token() }}" }
        ...
        .done(function(res) {
            ...
            $("#alert").html(`<div style="position: relative;background: #F0F5F7;border-radius: 8px;padding:5px;">
                   <b class="blinking" style="color:#d62727"> Merubah Data ini akan merubah Bidang Usaha yang sudah di pilih Perusahaan </b>
                </div>
                <br>`);
            $("#nama_bidang").val(res.data.nama_bidang);
        })
        .fail(function(err) {
            alert("error");
        })
    ...
    $('#submit').click(function(e) {
        ...
        apiLoading(true, btn);
        form.ajaxSubmit({
            url    : "{{url('api/admin/master-bidang-post')}}",
            type   : 'POST',
            headers: {
                "Authorization": "Bearer " + localStorage.getItem('jwt_token')
            },
